...
func CheckEthStorageProof(
	...
	// when prefixOrTotalHexLen = 0 means the value range in [0x00, 0x7f], the fist byte is the value it-self
	slotValueShort := rlp.HexToDecimal(api, shiftOut, 2, 2)
	slotValue[0] = api.Select(api.IsZero(prefixOrTotalHexLen), slotValueShort, slotValue[0]) //when prefixOrTotalHexLen = 0 high 248 bit = value rang in [0x00, 0x7f]
	slotValue[1] = api.Select(api.IsZero(prefixOrTotalHexLen), 0, slotValue[1])              // when prefixOrTotalHexLen = 0 low byte = 0
